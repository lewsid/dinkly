...
function dinkly_autoloader($class_name)
{
	...
	$core_model_file = $app_root . '/classes/models/core/' . $class_name . '.php';
		...
		foreach($contents as $file_or_dir)
		{
			if(is_dir($plugin_dir . '/' . $file_or_dir) && $file_or_dir != '..' && $file_or_dir != '.')
			{
				...
				if(is_dir($class_dir))
				{
					...
					$model_core_plugin_class = $class_dir . '/models/core/' . $class_name . '.php';
					...
					else if(file_exists($model_core_plugin_class))
					{
						require_once $model_core_plugin_class; 
						return true; 
					}
	...
	else if(file_exists($core_model_file))
	{
