...
abstract class DBObject
{
  ...
  {
    $this->db = new DBHelper(DBConfig::getDBCreds());
    $this->isNew = true;
  ...
  public function init($id)
  {
    ...
    if($this->db->arrResult != array())
    {
      $this->hydrate($this->db->arrResult, true);
    }
  }
  ...
  {  
    ...
    {
      if($reg[key($contents)] != NULL) {
        $this->$reg[key($contents)] = $record;
        next($contents);
      }
    }
    ...
    //Lighten up the object
    $this->db->arrResult = NULL;
    $this->db->Debug = NULL;
